...
function getFilteredCharacters() {
  ...
  return data.characters().filter(character => {
    const matchSearch =
      !keyword ||
      character.name.includes(state.searchText.trim()) ||
      character.id.toLowerCase().includes(keyword);

    const matchRole =
      state.roleFilter === "all" || character.role === state.roleFilter;
...
function renderDetailSidePanel() {
  if (state.selectedEchoSkillId) return renderEchoSkillDetailPanel();
  if (state.selectedEchoSetId) return renderEchoSetDetailPanel();
  return "";
}
